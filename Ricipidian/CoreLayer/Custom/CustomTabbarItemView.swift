...
class CustomTabbarItemView: CustomESTabBarItemContentView {
    ...
    public convenience init(specialWithAutoImplies implies: Bool) {
        ...
        highlightBackdropColor = UIColor(red: 22 / 255.0, green: 24 / 255.0, blue: 25 / 255.0, alpha: 1.0)
        if implies {
            let timer = Timer.scheduledTimer(timeInterval: 3.0, target: self, selector: #selector(CustomTabbarItemView.playImpliesAnimation(_:)), userInfo: nil, repeats: true)
